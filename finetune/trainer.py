...
"""
Customized trainer class for GLUE experiments of EarlyBERT.
...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, **kwargs):
        ...
        # `lottery_ticket_training` represents the efficient training stage in
        ...
        total_data_compute_time = 0.0
        total_whole_time = 0.0
        tr_loss = 0.0
        logging_loss = 0.0
        model.zero_grad()
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                tr_loss += self._training_step(model, inputs, optimizer, **kwargs)
                compute_end = time.time()
                total_compute_time += compute_end - compute_start
                with open(os.path.join(self.args.output_dir, 'time.log'), 'a+') as time_logger:
                    time_logger.write('total compute time {} for {} steps. average time per step: {}\n'.format(
                        total_compute_time, self.global_step+1, total_compute_time/(self.global_step+1)))
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    data_compute_end = time.time()
                    total_data_compute_time += data_compute_end - data_compute_start
                    with open(os.path.join(self.args.output_dir, 'time.log'), 'a+') as time_logger:
                        time_logger.write('total data + compute time {} for {} steps. average time per step: {}\n'.format(
                            total_data_compute_time, self.global_step+1, total_data_compute_time/(self.global_step+1)))
        ...
        whole_end = time.time()
        total_whole_time += whole_end - whole_start
        with open(os.path.join(self.args.output_dir, 'time.log'), 'a+') as time_logger:
            time_logger.write('\ntotal compute time {}\n'.format(total_compute_time))
            time_logger.write('total data + compute time {}\n'.format(total_data_compute_time))
            time_logger.write('total whole time {}\n'.format(total_whole_time))
