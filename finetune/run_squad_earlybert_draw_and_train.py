...
finetuning task for question-answering on SQuAD.
...
def train(args, train_dataset, model, tokenizer):
    ...
    if os.path.isfile(os.path.join(args.model_name_or_path, "optimizer.pt")) and os.path.isfile(
        ...
        # We skip loading the optimizer and scheduler states because we will
        # start over the training from the pre-trained weights.
        pass
    ...
    if os.path.exists(args.model_name_or_path):
        try:
            ...
            global_step = 1  # We start over the training from the pre-trained checkpoint
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="coefficient for the l1 loss of network slimming",
    )
    parser.add_argument(
        "--self_pruning_ratio",
        type=float,
    ...
    # global slimming_coef_records
    # slimming_coef_records = [[] for _ in range(config.num_hidden_layers)]
    tokenizer = AutoTokenizer.from_pretrained(
        args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
        do_lower_case=args.do_lower_case,
        cache_dir=args.cache_dir if args.cache_dir else None,
    )
    if args.load_from_pruned:
    ...
    # Create the training set to get the dataset length, not for training, so that
    # we can calculate at which step we will draw the ticket.
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # TODO: Currently the distributed training is not taken care of yet.
        # Load a trained model and vocabulary that you have fine-tuned
        # model = AutoModelForQuestionAnswering.from_pretrained(args.output_dir, config=new_config)  # , force_download=True)
        # tokenizer = AutoTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        # model.to(args.device)

    # Evaluation - we can ask to evaluate all the checkpoints (sub-directories) in a directory
    # NOTE: Currently, evaluate all checkpoints (sub-directories) in a directory
    #       is not supported yet. Only support evaluation after training.
    # TODO: Add support of
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # model = AutoModelForQuestionAnswering.from_pretrained(checkpoint, config=new_config)
            # model.to(args.device)
